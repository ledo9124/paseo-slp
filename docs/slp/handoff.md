...
Call `slp_request_handoff` once with `reason` and `context`. Supply the next objective, constraints, decisions and reasons, work done and remaining, evidence and artifact references, unresolved operations and next action. There is no checkpoint tool or periodic checkpoint duty.

The runtime saves this context before requesting the transfer, attaching identity, generation, accepted mail ids and a history cursor. The internal checkpoint files remain readable for interrupted-transfer recovery; agents do not manage them. Retries from the same source return the existing transfer without overwriting its context. After an aborted transfer, the reactivated source can submit a new handoff.

When Human requests a fresh Lead context, Supervisor asks Lead to perform this operation with the next task in mind. The runtime reports completion or blockage to Supervisor. A new Lead retains the existing slot and Peer relationships.

## Transfer sequence

An active agent starts the transfer of its own slot with the context in the same call. The hold is slot-scoped: Lead mail waits while Peers continue; destructive operations are held for the group. No automatic context threshold or provider compaction interception is enabled.
...
2. Read the supplied handoff context and capture history after its cursor. Record pending permissions, external effects and background jobs. Verify the source has stopped product execution.
...
The intended execution settings are the candidate's own stored config, copied from the source; the preparation state is the transfer journal and the generation state. There is no restoration step: the resolver reads `preparation` until the group pointer moves and `authorized` after, so a crash cannot land between the switch and the restoration. Never restore write access before the durable switch. The successor learns of the switch from the runtime: completion queues an `activation` mail keyed by the transfer, durable before the hold lifts, and the mailbox dispatches it ahead of anything that queued during the transfer. Without it a successor reads the shape of its next prompt, and a Codex successor declined product work that way ([evidence](evidence.md#findings-that-changed-code-or-the-contract)).
...
Notify the logical owner slot when a Peer returns. Do not close over the old Lead's agent ID. A Lead handoff does not cancel/recreate Peers or repeat their assignments. Transfer or resolve Paseo parentage before retiring the old parent so archive cannot cascade unexpectedly. A Peer handoff marks the old generation's handback transfer-in-progress, which suppresses every outcome — the stop, the close and an error are the transfer's, not a result — then supersedes it and registers a fresh record for the successor, so the slot keeps handing back from the successor's turns and never from the switch.
...
The register was shipped before the first Peer existed; retrofitting slot indirection after handoff would make every handoff produce spurious or lost handbacks. The register does not deliver anything itself: a fired handback becomes one mail in the owner slot's mailbox, with an id derived from the handback so re-queueing after a crash is a no-op, and the mail record carries the delivery state. A record lives as long as its Peer generation and fires on every turn the Peer returns, not once: the Lead's follow-up mail starts a new Peer turn, and that turn's last message is a handback too. Each delivery is numbered in the mail id. The runtime does not judge which turn ends carry work; a Peer that mailed the Lead mid-turn and then returned produces one extra wake, which is the accepted cost of never dropping a result left in its chat. A turn that leaves no assistant message queues nothing, and says so in the log rather than passing for a turn that never happened. An error or close ends the record.

A turn someone cut is not a return. `cancelAgentRun` marks the agent before it interrupts — after it, the run has already settled and the idle state the register reacts to is gone — and only an explicit user cancel marks it, because the same call is the prelude to a reload, replace or rewind. The register reads that mark off the idle state and fires `cancelled`, which is not terminal: the assignment stands and the Peer hands back again on a later turn. A cancelled turn hands back **even with no last message**, the one case where silence is worse than an empty delivery. Without this the owner is told its Peer returned, over whatever the Peer last happened to narrate mid-turn, and reads a status ping as a result; see [evidence](evidence.md#field-run).

The record marks a running turn on disk, so after a daemon restart the owner is told `interrupted` only for a Peer that was mid-turn; a Peer idle between turns is re-armed silently. A Peer whose creation the daemon died inside is retired and its handback abandoned.
...
The handoff call captures the daemon timeline epoch and sequence with the supplied context. Stopping reads the subsequent rows into a bounded, durable tail. A timeline that changes epoch between request and stop blocks the transfer. Neither adapter's history path is used yet, so missing provider history cannot be recovered. A tool call that started before the cursor and finished after it is updated in place at its old sequence and does not appear in the tail; the source must account for unresolved operations in its handoff context.
