...
The register was shipped before the first Peer existed; retrofitting slot indirection after handoff would make every handoff produce spurious or lost handbacks. Two rules follow from what the register can and cannot know:

- Until the slot mailbox lands, an owner that is busy and cannot be steered keeps the handback owed and receives it as its next turn. The declined attempt leaves no journal receipt, so the retry is never mistaken for an uncertain outcome.
- A daemon restart loses every in-flight turn, and the register cannot tell an interrupted Peer from one that never started. Every armed handback is therefore reported to the owner as `interrupted` at boot and stays armed, so a re-driven Peer still hands back once. A Peer whose creation the daemon died inside is retired and its handback abandoned.
