...
Caller-aware policy is new code, not an extension. The existing tool policy keys on provider id alone, so two SLP agents on one provider resolve identical policy, and it fails open when no policy is found — do not reuse that predicate for a role denial. `slp/authority.ts` is the policy table: which tools a role sees, and which relation (own Peer, owner, Supervisor, Lead, self) each agent-targeting tool accepts. The tool catalog hides denied tools per caller and checks the target of every tool whose parsed input names an `agentId`; the pending-permission listing stays daemon-wide.
...
| `agent/tools/paseo-tools.ts`   | Caller-aware inputs and defaults; the catalog asks the SLP authority which tools a caller sees and checks the target of agent-addressed tools; `send_agent_prompt` between members is routed to the slot mailbox                                                    | Nothing further until the client surface (PR 7)                                                           |
