...
A supervised Lead's turn ends in a chat Human never reads. When such a turn ends and the Lead queued no mail to the Supervisor during it, the runtime delivers the Lead's last assistant message to the Supervisor slot as `report` mail, the way a Peer's last message is its handback; a turn the Lead reported on itself adds nothing. `slp/reports.ts` owns it; it is in-memory (a turn lost to a restart is the Supervisor's to notice) and stays quiet while the group is held.
...
Reuse the shared native/MCP tool catalog with caller-aware SLP policy. A provider's own multi-agent tools cannot be switched off for members on codex-cli 0.153.4: with `features.multi_agent_v2` disabled Codex offers the v1 `multi_agent_v1__*` tools instead, whatever `features.multi_agent` says, and a member spawned an untracked Codex agent through them (see [evidence](evidence.md#findings-that-changed-code-or-the-contract)). Members are created with the host's own Codex features, the identity block names the native tools as off-limits, and the Lead report relay covers the case where a member reports through the wrong tool. Lead may create bounded Peers; Supervisor and Peer may not. Default result acceptance remains Lead's engineering judgment, expressed with evidence rather than inferred from lifecycle.
