...
The daemon advertises `features.slpGroups` in `server_info` while the feature is on, and the app shows nothing SLP-related without it. Two RPCs and one push carry the whole surface: `slp.group.initialize.request` (workspace, mode, launch config, first message with its client message id), `slp.group.get.request` (by workspace), and `slp.group.update`, pushed to every session after any durable change to a group, one of its transfers or its mail. The payload is one `SlpGroupSummary` per group, computed by `SlpService.summarize` from the group record, the transfer journal and the mailbox; the app keeps it as the daemon's projection and derives per-agent facts from it. Role, state, phase and mode travel as strings, not enums, so an older app keeps parsing when the daemon learns a value; the closed enums live in `slp/store.ts`. Initialization refusals are answered in the response with the runtime error's name as the code, never thrown through `rpc_error`, so the app can show the reason beside the form.
