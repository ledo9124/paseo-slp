...
| `agent/agent-manager.ts`       | `runForegroundMutation` serializes cancel, steer admission and `admitForegroundTurn`; archive, snapshot archive and delete consult the `DestructiveOperationGate` before their lifecycle lane; `streamAgent` consults the `TurnAdmissionGate` at the run-slot claim so a retired generation cannot start a turn on any route; `closeAgent` reaches no cascade | Provider preparation policy at the launch boundary (PR 5b)               |
