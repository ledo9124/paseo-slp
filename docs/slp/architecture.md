...
A supervised Lead's turn ends in a chat Human never reads. The runtime delivers its last nonempty assistant message to the Supervisor slot as `report` mail, including when the Lead sent progress earlier in the turn. The last message is the report; Lead needs no additional tool call or rewritten summary. Explicit mail remains available for contact during a turn. The notice identifies a turn ending, not task completion; Supervisor judges whether the content is a result, a question or a blocker. `slp/reports.ts` owns the relay and stays quiet while the group is held. Turn tracking is in-memory: a restart before the report is enqueued can still lose it. Once enqueued, it follows the durable slot mailbox and waits for a busy Supervisor without interrupting it.
...
## Role launch defaults

Host role settings seed new groups. Before sending the first message, edit Supervisor and Lead independently in the composer; these selections affect only that group. The daemon snapshots the resolved launch values with initialization so recovery does not consult changed defaults. Editing a running session's model uses its existing model control, and its successor inherits the session's effective configuration. The `slpRoleOverrides` capability gates the per-role composer on older hosts.

SLP members use Paseo for delegation. The launch policy removes Codex native collaboration and denies Claude's native `Agent`/`Task` delegation, preserving other provider settings. Single-agent sessions keep their provider's native capabilities.
...
`features.slp.handoff` in `config.json` (or `PASEO_SLP_HANDOFF`) enables explicit same-role handoff by default. Setting it false hides the handoff tools and the role's `## Handoff` section. In-flight transfers still finish or recover. See [handoff](handoff.md#handoff-context) for the single-call interface; automatic compaction interception remains unimplemented.
...
Enumerate every delivery route into a slot — the WebSocket send, the MCP send tool, schedule fires, finish notifications — and decide each one's policy explicitly. Decided so far: `send_agent_prompt` from a group member to a group member is mail to the target's slot; a Peer does not have that tool, and every turn it returns is mail from the handback register. Still open, and still interrupting a busy recipient: a Human WebSocket send addressed to a member, a send from an agent outside the group, and schedule fires. A human send additionally clears the recipient's pending permissions; SLP mail must not inherit that flag or a routine message would deny a Peer's pending tool approval.
