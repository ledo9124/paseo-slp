...
Status: target design, with the group store, fixed mode, initialization, boot recovery, destructive-operation gate, prompt composition, the tool precondition, Peer creation and the handback register implemented in `packages/server/src/server/slp/`. Every other section is unimplemented. See [the documentation map](README.md) for scope and [the implementation plan](implementation-plan.md) for delivery gates.
