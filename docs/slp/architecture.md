...
Status: target design. The decisions below were taken against a full survey of the current daemon and every code reference in this document was verified on `main`; no SLP runtime is implemented. See [the documentation map](README.md) for scope and [the implementation plan](implementation-plan.md) for delivery gates.
