...
try {
  ...
  {
    console.log("Test 2: falls back to homedir/.paseo-slp when PASEO_HOME is unset");
    delete process.env.PASEO_HOME;

    assert.strictEqual(resolvePaseoHomePath(), join(homedir(), ".paseo-slp"));
    assert.strictEqual(resolvePaseoWorktreesDir(), join(homedir(), ".paseo-slp", "worktrees"));
