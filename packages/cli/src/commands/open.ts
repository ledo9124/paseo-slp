...
function findDesktopApp(): string | null {
  const bundledApp = process.env.PASEO_SLP_DESKTOP_APP?.trim();
  if (bundledApp) return existsSync(bundledApp) ? bundledApp : null;
  if (process.platform === "darwin") {
    const candidates = [
      "/Applications/Paseo SLP.app",
      path.join(homedir(), "Applications", "Paseo SLP.app"),
    ];
  ...
  if (process.platform === "linux") {
    const candidates = [
      "/usr/bin/Paseo SLP",
      "/usr/bin/paseo-slp",
      "/opt/Paseo SLP/Paseo SLP",
      path.join(homedir(), "Applications", `Paseo-SLP-${process.arch}.AppImage`),
    ];
  ...
  if (process.platform === "win32") {
    ...
    const candidate = path.join(localAppData, "Programs", "Paseo SLP", "Paseo SLP.exe");
...
function launchDesktop(args: string[]): void {
  if (process.env.PASEO_DESKTOP_CLI === "1") {
    throw new Error("Cannot open Paseo SLP Desktop while running in desktop CLI passthrough mode.");
  }

  const desktopApp = findDesktopApp();
  if (!desktopApp) {
    throw new Error(
      "Paseo SLP desktop app not found. Install it from https://github.com/ledo9124/paseo-slp/releases",
    );
  }
