...
async function main() {
  ...
  try {
    ...
    const helper = fs.statSync("/opt/Paseo SLP/chrome-sandbox");
    ...
    await smokePackagedDesktopApp({ appPath: "/opt/Paseo SLP", expectedSandbox: true });
    ...
    const desktopEntry = fs.readFileSync(path.join(appDir, "Paseo-SLP.desktop"), "utf8");
    ...
    const appPath = fs.existsSync(path.join(tarDir, "Paseo SLP"))
      ? tarDir
